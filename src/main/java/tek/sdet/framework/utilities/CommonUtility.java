...
public class CommonUtility extends BaseSetup {
	...
	public void deselectByVisibility(WebElement ele, String visibleText) {
		Select select = new Select (ele);
		select.deselectByVisibleText(visibleText);
	}
	...
	public void HighlightElement (WebElement ele) {
		...
		js.executeScript("arguments[]style.border = '3px solid red'", ele);
	...
	public void switchwindow(String pageTile) {
		String currentWindow = getDriver().getWindowHandle();
		Set<String>handles = getDriver().getWindowHandles();
		for(String winHandle: handles) {
			String currentWindowTitles = getDriver().switchTo().window(winHandle).getTitle();
			if(currentWindowTitles.equals(pageTile)) {
				break;
			}else {
	...
	public void selectcalendarDataWithJS(String data, WebElement element) {
		JavascriptExecutor js = ((JavascriptExecutor)getDriver());
	...
	public void scrollDownPagewithJS() {
		JavascriptExecutor js = ((JavascriptExecutor)getDriver());
		js.executeScript("window .scrollTo(0, document.boday.scrollHeight)");
	}
